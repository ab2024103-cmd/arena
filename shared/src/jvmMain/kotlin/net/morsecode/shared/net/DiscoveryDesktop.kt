...
/** JmDNS-backed discovery for desktop (Section 3). */
actual class MdnsDiscovery actual constructor(scope: CoroutineScope) : JmDnsDiscoveryBase(scope) {
    ...
    actual override fun start(self: SelfProfile, port: Int, roomId: String?) {
        super.start(self, port, roomId)
    }

    actual override fun updateRoom(roomId: String?) {
        super.updateRoom(roomId)
    }
    ...
    actual override fun refreshNow() {
        super.refreshNow()
    }
